...
// Watermark via pseudo-éléments: aucun JS requis
...
// Force reload hero background on mobile by removing then re-adding
;(function () {
  const hero = document.querySelector('.hero')
  const isMobile = window.matchMedia('(max-width: 600px)').matches
  if (!hero || !isMobile) return
  const original = "url('GUSARK.PNG')"
  const cacheBust = `url('GUSARK.PNG?v=${Date.now()}')`
  hero.style.backgroundImage = 'none'
  requestAnimationFrame(() => {
    hero.style.backgroundImage = cacheBust
    setTimeout(() => {
      hero.style.backgroundImage = original
    }, 120)
  })
})()
